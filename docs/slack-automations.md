...
3. Also report **idle CC capacity** (floor(shares/100) − short calls), **accumulation CSP status** (AMD/META: open / waiting-earnings / blocked-collateral / graduated), and **index CSP sleeve** status (free cash vs put collateral; prefer new puts on RSP then ITOT then SPY; ~$2k BP buffer). One slot at a time for accumulation + index CSPs combined — do not stack if combined collateral breaches BP buffer. For idle CC: propose **full idle** per symbol with unused capacity at sleeve entry Δ when **today’s** gates PASS — no phase-in / no “start with 2–4 contracts.” **Re-evaluate every eligible symbol every run** (no permanent skip lists; prior-day FAIL does not skip today). Skip only when **this run’s** economic spread gate fails, or earnings blackout applies.
...
- Honor docs/strategy.md including:
  ...
  - idle CC fill: **full idle** per symbol with unused capacity at sleeve entry Δ when **today’s** gates PASS (no phase-in); **re-check every eligible symbol every EXECUTE** — no permanent skip lists or “phased later” queues from prior runs; skip only when **this run’s** economic spread fails, or earnings blackout; share coverage required. Index CSP opens only with free cash + ~$2k BP buffer; prefer RSP then ITOT then SPY for new puts
