...
| Open strategy canvas | Open `canvas/index.html` (scorecard · equities · options) |
| Refresh canvas stats | Prompt: `Refresh the strategy canvas snapshot` (add **and publish to GCP** to commit + deploy) |
...
| Slack daily report / continue-stop | Edit prompts in `docs/slack-automations.md`, paste into [Automations](https://cursor.com/automations) — daily check publishes canvas before Slack; continue/stop republishes after trades → https://storage.googleapis.com/agentic-trading-canvas/index.html |
